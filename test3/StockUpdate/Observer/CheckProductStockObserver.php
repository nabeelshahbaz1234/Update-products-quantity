...
class CheckProductStockObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer): void
    {
        ...
        // Create a mapping of product IDs to their respective quantities from the API response
        $productQuantities = [];
        foreach ($quantityDataList as $quantityData) {
            $productQuantities[$quantityData['product_id']] = (float)$quantityData['quantity'];
        }
        ...
        foreach ($products as $product) {
            ...
            // Check if the product ID exists in the API response data
            if (isset($productQuantities[$productId])) {

                $newQuantity = $productQuantities[$productId];
